...
function searchdata_to_search(act,key,cat,keyinput,pos,keys,suggesttype,suggesturl,ref,rcount,type){
    if(document.images){
        ...
        var custid=getCookie_one("customerid","user");
        var permid=getCookie_one("__permanent_id");
...
function getCookie_one(name,type){
  ...
  var temp ;
  var find_name;
  if(type=="user" || type=="ddoy" || type=="login"){
    if(type == "user"){find_name = "user.com=";}
    if(type == "ddoy"){find_name = "ddoy=";}
    if(type == "login"){find_name = "login.user.com=";}
    var dangdangcookie="";
     for(i=0;i<cookies.length;i++){
      ...
      for(j=0;j<cookies.length;j++){
          if(cookies[j].indexOf("login.user.com=")>0){
      ...
      for(i=0;i<cookies.length;i++){
         if(cookies[i].indexOf("user.com=")<0){
      ...
      for(j=0;j<cookies.length;j++){
          if(cookies[j].indexOf("user.com=")==1){
              if(name=="customerid"){
...
{
    var usernick=getCookie_one("show_name","user");
    ...
    if(screensize >= minsize){
        ...
        document.cookie = "ddscreen=2;Domain=user.com";
    }
    //narrow_screen
    if(screensize < minsize){
        ...
        document.cookie = "ddscreen=1;Domain=user.com";
    }
    ...
    if(validatedflag=="1")
        marks = "<a name=\"ddnav_verifymail\" href=\"https://login.user.com/VerifyEmail.aspx\">(δ��֤)</a>";
    ...
    if(usernick!='' && aspxauth!=''){
        userlink = '<a href="http://myhome.user.com/" class="login_link" target="_blank"><b>'+usernick+'</b></a>';
        nkname.innerHTML='<span class="hi">Hi��'+userlink+''+marks+chakanlk;
    }else if(usernick!=''){
        userlink = '<a href="http://myhome.user.com" target="_blank" class="login_link"><b>'+usernick+'</b></a>';
...
function message_data(data){
    ...
    if(data["errorCode"]=="200"){
        ...
        var znx_count = '<a href="http://message.user.com/mymessage.php#1" target="_blank" class="head_znx_a" id="znx_channel" onmouseover="showgaoji(\'znx_channel\',\'znx_items\');" onmouseout="hideotherchannel(\'znx_channel\',\'znx_items\');">վ����(<span>'+znx_sum+'</span>)</a>';
        var znx_jytx = '<li><a href="http://message.user.com/mymessage.php#1" target="_blank">��������(<span>'+jytx_num+'</span>)</a></li>';
        var znx_zhtx = '<li><a href="http://message.user.com/mymessage.php#2" target="_blank">�˻�����(<span>'+zhtx_num+'</span>)</a></li>';
        var znx_sqxx = '<li><a href="http://message.user.com/mymessage.php#5" target="_blank">������Ϣ(<span>'+sqxx_num+'</span>)</a></li>';
        var znx_fwtz = '<li><a href="http://message.user.com/mymessage.php#3" target="_blank">����֪ͨ(<span>'+fwtz_num+'</span>)</a></li>';
        var znx_scjtx = '<li><a href="http://message.user.com/mymessage.php#4" target="_blank">�ղؼ�����(<span>'+scjtx_num+'</span>)</a></li>';
...
nkname.innerHTML='���ã���ӭ���ٵ�����<a href="http://reco.user.com/">����Ϊ���Ƽ���</a>[��<a href="http://login.user.com/Signin.aspx" name="sign_in" target="_self" class="blue12a">��¼</a>/<a href="http://login.user.com/Register.aspx" target="_self" name="Register" class="blue12a">ע��</a>]';
}
...
function change_city(key,value){
    ...
    sug_gid('city_list').style.display = "none";
    document.cookie = "city_id=cityid_"+key+";Domain=user.com";
...
function change_area(key,value){
    ...
    if(key!=174){
    document.cookie = 'dest_area='+escape('country_id=9000&province_id='+key+'&city_id =0&district_id=0&town_id=0')+';Domain=user.com;path=/;expires='+oDate.toGMTString();
    }
    hidden_area_list();
}
